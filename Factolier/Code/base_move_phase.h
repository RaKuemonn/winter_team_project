...
template<typename Enemy>
class Move_Phases final
{
private:
    using Move_State = Move_Phase<Enemy>;
    using ptr_Move_State = Move_State*;
...
public:
    Move_Phases& set(ptr_Move_State move_)
    {
        const int index = static_cast<int>(find_move_index(move_));
        if (index != -1)
        {
    ...
    Move_Phases& add_transition(ptr_Move_State from, ptr_Move_State to, const transition_func& Is_)
    {
    ...
    Move_Phases& set_initialize(ptr_Move_State move_, const initialize_func& initialize)
    {
        try_add_move(move_).initialize = initialize;
        return *this;
    }

    Move_Phases& set_finalize(ptr_Move_State move_, const finalize_func& finalize)
    {
        try_add_move(move_).finalize = finalize;
        return *this;
    }
