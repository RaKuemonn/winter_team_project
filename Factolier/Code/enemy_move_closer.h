...
private:
    ...
    void init_in_area(const float elased_time_);                // 初期化
    void deinit_in_area(const float elased_time_);              // 終了
    void find_in_area(const float elapsed_time_);               // エリア内の探索位置を決定
    void move_in_area(const float elapsed_time_);               // 探索位置に向けて移動 (自分の前方方向に進む)
    void look_in_area(const float elapsed_time_);               // 探索位置に向く
    void idle_in_area(const float elapsed_time_);               // 待機                 (なにもしていない)


    void seek_to_target(const float elapsed_time_);             // 目標を見つけた
    void init_to_target(const float elapsed_time_);             // 初期化
    void deinit_to_target(const float elased_time_);            // 終了
